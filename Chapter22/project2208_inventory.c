...
// Load the database from a specifieed file. 
...
void dump(const struct part inv[], int *np);
void restore(struct part inv[], int *np);
	...
	for (;;) {
		...
		switch(code) {
			...
			case 'd': dump(inventory, &num_parts);
				break;
			case 'r': restore(inventory, &num_parts);
				break;
			case 'q': return 0;
...
void dump(const struct part inv[], int *np)
{
	...
	fwrite(inv, sizeof(struct part), *np, fp);
	fclose(fp);
...
void restore(struct part inv[], int *np)
{
	...
	*np = fread(inv, sizeof(struct part), MAX_PARTS, fp);
